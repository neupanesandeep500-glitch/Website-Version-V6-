...
def settings_panel():
    """Read-only data-source status. Uploading/syncing now happens only
    through the login-gated /admin panel — kept off the public dashboard
    for both security (no one else can overwrite the live workbook) and
    speed (no upload widgets to load for ordinary visitors). The admin
    login link itself is intentionally not shown here either — admin
    reaches it by navigating to /admin/login directly."""
    return dbc.Card(
        dbc.CardBody([
            html.H5([html.I(className="bi bi-info-circle me-2"), "Data Source"], className="mb-2"),
            html.Div(id="load-status", className="small text-muted"),
        ]),
        className="shadow-sm",
    )
...
app.layout = dbc.Container(fluid=True, children=[
    ...
    dbc.NavbarSimple(
        brand=html.Span([
            # Nepal flag — served from /assets/nepal_flag.png (Dash's built-in
            # static-assets route). Always the leftmost element, before the
            # admin-uploadable logo and before the title text.
            html.Img(src="/assets/nepal_flag.png", height="32px",
    ...
    dbc.Row([
        ...
        dbc.Col(md=9, children=[
            ...
            dbc.Tabs(id="main-tabs", active_tab="overview", className="main-tabs-nav", children=[
                dbc.Tab(label=[html.I(className="bi bi-speedometer2 me-1"), "Overview"], tab_id="overview"),
                dbc.Tab(label=[html.I(className="bi bi-lightning-charge-fill me-1"), "Power Plants"], tab_id="plants"),
                dbc.Tab(label=[html.I(className="bi bi-diagram-3-fill me-1"), "Transmission Line"], tab_id="transmission"),
                dbc.Tab(label=[html.I(className="bi bi-journal-text me-1"), "GoN Studied Projects"], tab_id="gon_study"),
                dbc.Tab(label=[html.I(className="bi bi-x-octagon-fill me-1"), "License Cancelled"], tab_id="cancelled"),
                dbc.Tab(label=[html.I(className="bi bi-graph-up-arrow me-1"), "Growth Trends"], tab_id="growth"),
                dbc.Tab(label=[html.I(className="bi bi-map-fill me-1"), "GIS Map"], tab_id="gis"),
                dbc.Tab(label=[html.I(className="bi bi-bar-chart-line-fill me-1"), "Comparative Charts"], tab_id="compare"),
                dbc.Tab(label=[html.I(className="bi bi-table me-1"), "Data Table"], tab_id="table"),
            ]),
    ...
    html.Footer(className="site-footer", children=[
        dbc.Row([
            ...
            dbc.Col(md=4, className="text-md-end", children=[
                html.Div("👥 …visitors", id="visitor-counter", className="small"),
            ]),
        ]),
...
def handle_data_source(_init, _poll):
    ...
    return (status_msg, types, statuses, provinces, [y_lo, y_hi])
...
def update_kpis(f_type, f_status, f_province, f_capacity, f_year, f_search,
    ...
    n = len(recs)
    total_mw = sum(r["capacity_mw"] or 0 for r in recs)
    n_operating = sum(1 for r in recs if r["status"] == "Operating")
    n_transmission = sum(1 for r in recs if r["type"] == "Transmission Line")
    total_km = sum(r["line_length_km"] or 0 for r in recs if r["type"] == "Transmission Line")
    cards = [
        kpi_card("Total Projects", f"{n:,}", "matching current filters", "#1565c0"),
        kpi_card("Total Capacity", f"{total_mw:,.1f} MW", "power plants + solar/wind/etc.", "#2e7d32"),
        kpi_card("Operating", f"{n_operating:,}", "already commissioned", "#e65100"),
        kpi_card("Transmission Lines", f"{n_transmission:,}", f"{total_km:,.0f} km total", "#6a1b9a"),
    ]
...
def update_ticker(_status, _poll):
    if not ss.get_marquee_enabled():
        return None
    return render_ticker_bar(STATE["loader"])
...
def render_tab(tab, f_type, f_status, f_province, f_capacity, f_year, f_search,
    ...
    if loader is None or loader.error or not loader.records:
        return dbc.Alert(
            "Dashboard data is not available right now. Please check back shortly.",
...
def build_ticker_segments(loader):
    ...
    and latest-connected projects, computed over the FULL dataset (not the
    current filter selection), same as the original."""
    all_recs = loader.records
    ...
    segs = [(f"⚡ ACTIVE POWER PLANTS — {len(plants):,} • "
             f"{sum(r['capacity_mw'] or 0 for r in plants):,.0f} MW", "#ffd166")]
    for st in de.STATUS_ORDER:
        sel = [r for r in plants if r["status"] == st]
        if sel:
            segs.append((f"{STAGE_SHORT.get(st, st)}: {len(sel):,} • "
                         f"{sum(r['capacity_mw'] or 0 for r in sel):,.0f} MW",
                         de.STATUS_COLORS.get(st, "#c8d3e8")))
    km_all = sum(r["line_length_km"] or 0 for r in txs)
    segs.append((f"🔌 TRANSMISSION — {len(txs):,} lines • {km_all:,.0f} km • "
                 f"{sum(r['capacity_mw'] or 0 for r in txs):,.0f} MW", "#7fd1ff"))
    segs.append((f"🏛 GoN STUDY PROJECTS — {len(gons):,} • "
                 f"{sum(r['capacity_mw'] or 0 for r in gons):,.0f} MW", "#f4b860"))
    if tcs:
        segs.append((f"Technical Clearance — {len(tcs):,} • "
                     f"{sum(r['capacity_mw'] or 0 for r in tcs):,.0f} MW", "#9fb3c8"))
    segs.append((f"🚫 LICENCE CANCELLED — {len(canc):,} • "
                 f"{sum(r['capacity_mw'] or 0 for r in canc):,.0f} MW", "#ff8a80"))
    ...
    for tlabel, icon, sel in (
        ...
        if not sel:
            continue
        segs.append((f"{icon} {tlabel} in operation: {len(sel):,} • "
                     f"{sum(r['capacity_mw'] or 0 for r in sel):,.0f} MW", "#a5f3c4"))
    ...
    segs.append((f"📈 Capacity added {ty_} (01-01 → {ty_}-{tm_:02d}-{td_:02d}): "
                 f"{mw_cur:,.0f} MW ({n_cur:,} proj)  vs  same period {ty_-1}: "
    ...
    segs.append((f"📅 Connected in {ty_-1} (full year): {len(last_full_year):,} projects • "
                 f"{sum(r['capacity_mw'] or 0 for r in last_full_year):,.0f} MW", "#ffe08a"))

    yr_sel = cur_sel
    segs.append((f"🆕 In operation since {ty_}-01-01: {len(yr_sel):,} projects • "
                 f"{sum(r['capacity_mw'] or 0 for r in yr_sel):,.0f} MW", "#ffe08a"))
    ...
    largest_this_year = sorted(cur_sel, key=lambda r: r["capacity_mw"] or 0, reverse=True)[:3]
    largest_ids = {id(r) for r in largest_this_year}
    for r in largest_this_year:
        segs.append((f"🏆 Largest connected {ty_}: {r['project'][:34]} — "
    ...
    latest = [r for r in latest_candidates if id(r) not in largest_ids][:3]
    for r in latest:
        segs.append((f"🔌 Latest connected {ty_}: {r['project'][:34]} — "
                     f"{de.fmt_mw(r['capacity_mw'])} MW • {_admin_units_str(r)} • "
...
def render_ticker_bar(loader):
    if loader is None or not loader.records:
        return None
    try:
        segs = build_ticker_segments(loader)
    except Exception:
        traceback.print_exc()
        return None
    if not segs:
...
def render_gis(loader, recs, f_crs=None, show_boundary=True, show_pa=False):
    if not de.GIS.loaded:
        return dbc.Alert(
            "The map layer is being updated and will be back shortly. "
    ...
    return dcc.Graph(id="gis-map", figure=fig, config={"scrollZoom": True})
